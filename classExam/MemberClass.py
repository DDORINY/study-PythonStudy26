...
class MemberManager: # 객체를 담당하는 클래스 (사용법:변수= MemberManager()생성)
    ...
    # 클래스 작업시 (def __init__)필수!!
    def __init__(self, file_name="members.txt"): # 객체생성시 만드는 기본값을 (생성자)
        self.file_name = file_name # 객체의 파일이름을 정의한다.
        self.members = []          # 객체의 members 리스트를 만든다.
        self.session = None        # 객체에 세션변수를 만들고 기본값으로 None 처리(정수)
        self.load_members()        # 아래에 선언된 메서드를 호출한다.

    # ===============================
    # 파일 로드
    # ===============================
    def load_members(self): # 앞으로 만들 메서드는 ()괄호 안에 self가 필수!!
        self.members = [] # 빈 배열로 생성 (이전에 리스트가 남아 있을 수 있음)

        if not os.path.exists(self.file_name): # 동일 디렉토리에 파일명이 없으면?
            self.save_members()                # save_members()메서드를 호출( open()으로 파일 생성)
            return                             # load_members()메서드를 빠져나와라

        with open(self.file_name, "r", encoding="utf-8") as f:
        #          members.txt  읽기전용   한글처리         -> f라는 변수에 넣어라
            for line in f: # f변수에 있는 파일객체를 줄단위로 반복함
                data = line.strip().split("|") # -> 1줄 읽은 값을 엔터제거, |를 기준으로 잘라
                # ["kkw","1234","김기원","admin","True"]
                data[4] = True if data[4] == "True" else False
                # 리스트 5번째 값이 문자열로 True이면 불타입 True로 변경 아니면 False
    ...
    # members 2차원 리스트 값을 파일에 덮어쓴다.
    # why? 파일처리는 수정하지 않은ㄴ다. [r|읽기전용][w|덮어쓰기][a|마지막 추가]
        with open(self.file_name, "w", encoding="utf-8") as f:
            #      members.txt   덮어쓰기     한글처리        -> f라는 변수에 넣어라
            for m in self.members: # 메모리에 있는 members 2차원 리스트를 1줄씩 가져와
                # m 변수에 넣어라
                # m =     ["kkw","1234","김기원","admin","True"]
                f.write(f"{m[0]}|{m[1]}|{m[2]}|{m[3]}|{m[4]}\n")
                # m =     kkw|1234|김기원|admin|True 엔터 -> write 저장
    ...
    def member_add(self): # self는 클래스의 객체 주소
        ...
        for m in self.members: # 2차원배열인 members에 1차원 리스트를 가져와 (1줄)
            if m[0] == uid:
                print("이미 존재하는 아이디입니다.")
                return # member_add 메서드를 빠져나간다.

        # 중복 id가 없으면 아래쪽 코드실행 -> else:로 처리해도 되나 들여쓰기가 필수
        pw = input("비밀번호 : ")
        name = input("이름 : ")
        ...
        # idx, member
            if m[0] == uid: # for문 중에 같은 아이디가 있으면
                ...
                if m[1] == pw: # member[]
                    ...
                    if m[3] == "admin": # 관리자이면
                        self.member_admin() # 메서드를 호출한다. (관리자 메뉴)
                    return # member_login() 메서드를 빠져나온다.
                else: # if m[1] == pw:가 False이면
                    print("비밀번호 오류")
                    return # member_login() 메서드를 빠져나온다.
    ...
    # 관리자 기능
    # ===============================
    def member_admin(self): #로그인시 admin = role이면 진입
        print("\n[관리자 메뉴]")
        print("1. 비밀번호 변경")
        print("2. 블랙리스트")
        print("3. 권한 변경")
        ...
        uid = input("대상 아이디 : ") # 대상 id 찾는 입력

        for m in self.members: # members의 2차원 배열을 반복
            if m[0] == uid:
                if sel == "1": # 비밀번호 변경
                    m[1] = input("새 비밀번호 : ")
                elif sel == "2": # 블랙리스트 처리
    ...
    def member_logout(self):
        self.session = None # session 값에 있는 인덱스를 None처리를 한다.
        print("로그아웃 완료")
    ...
    def member_modify(self):
        if self.session is None: #session의 값이 None이면 로그인 해라
            print("로그인 필요")
            return
