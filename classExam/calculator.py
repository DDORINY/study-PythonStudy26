...
# 클래스로 구현하면 여러명이 접근해서 사용할 수 있다. -->FLASK, fastAPI

# 변수 선언 시 글로벌 영역에 선언하면 : session-> 값을 공용으로 사용
# 클래스가 아닌 일반 사항으로 처리하면 1명이 로그인하면
# 다른 사람이 접속할 때 이미 로그인한 상태가 됨

result = 0 # result 결과 값이라는 영문
result2 = 0
...
# 여러명이 add라는 함수를 사용하려면 그 수만큼 함수와 변수를 생성
