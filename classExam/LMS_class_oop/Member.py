...
# MemberDTO,MemberVO라는 이름으로 사용이된다.
#----------------------------------------------------------------------------------------------------------------------
# DTO | Data Transfer Object,데이터 전송 객체
# VO | Value Object, 값 그 자체
#----------------------------------------------------------------------------------------------------------------------

# 회원 각각의 자료를 리스트가 아닌 변수에 담아 제공
class Member: # class는 무조건 대문자로 시작한다.
    ...
    # 사용방법 member = Member() → 객체를 생성하여 변수에 연결
    # 이름 : member.id
    # 암호 : member.pw
    ...
    def from_line(cls,line): # line : 메모장의 1줄 문자열
        ...
        #     변수들에게 넣음    ←     1줄 문자열을 엔터를 지우고 |로 잘름
        return cls(uid,pw,name,role,active == "True")

    # 사용법
    # m = Member(uid,pw,name,role,active) → 권장하지 않음
    # → self를 사용하는 방법 (객체변수)

    # m = Member.from_line(line) → 객체생성 책임을 클래스가 담당 [권장]
    # → cls를 사용하는 방법 (클래스변수)

    # ------------------------------------------------------------------------------------------------------------------
    # 면접! 자주하는 질문!! (직렬화, 역직렬화의 정의)
    #--------------------
    # 직렬화(Serialization) | 객체 → 저장 가능한 형태
    #       member.to_line()

    # 역질렬화(Deserialization) | 저장된 데이터 → 객체 (@classmethod)
