        ...
        while scoreRun:
            ...
            ############미완성#######################################################################################################
                ...
                if gropes[session] == "professor" or "admin" or "office":
                    ...
                    if select == "1":
                        for i in range(0, len(pythonScores) + 1):
                            if pythonScores.get(i):
                                print(dedent(f"""\
                                학번 : {stuIdxs[i]}
                                이름 : {names[i]}, 
                                파이썬 점수 : {pythonScores.get(i + 1)}점 {pythonGradeScores.get(i + 1)}등급
                                데이터 점수 : {dataBaseScores.get(i + 1)}점 {dataBaseGradeScores.get(i + 1)}등급
                                프론트 점수 : {wwwScores.get(i + 1)}점 {wwwGradeScores.get(i + 1)}등급
                                """))
